		...
		<ul id="ul_object" class="nav nav-list bs-sidenav">
			...
			foreach ( $allObject as $object_li ) {
				...
				if ($object_li->getId () == $object->getId ()) {
					echo '<li class="cursor li_object active" ><a href="index.php?v=d&p=panel&m=chromecast&object_id=' . $object_li->getId () . '" style="position:relative;left:' . $margin . 'px;">' . $object_li->getHumanName ( true ) . '</a></li>';
				} else {
					echo '<li class="cursor li_object" ><a href="index.php?v=d&p=panel&m=chromecast&object_id=' . $object_li->getId () . '" style="position:relative;left:' . $margin . 'px;">' . $object_li->getHumanName ( true ) . '</a></li>';
				}
			}
			?>
        </ul>
...
?>
<i class='fa fa-picture-o cursor tooltips pull-left'
	id='bt_displayObject'
	data-display='<?php echo $_SESSION['user']->getOptions('displayObjetByDefault')?>'
	title="Afficher/Masquer les objets"></i>
</br>
<!-- 
<a class="btn btn-success onall"><i class="fa fa-spinner"></i> Allumer
	tous </a>
<a class="btn btn-danger offall"><i class="fa fa-power-off"></i>
	Eteindre tous</a>
 -->	
<a href="index.php?v=d&m=chromecast&p=chromecast" class="btn btn-info"><i
	class="fa fa-list-alt"></i> Equipements</a>
<div class="pull-right"></div>
</br>
<?php
echo '<div class="div_displayEquipement" style="width: 100%; margin-top: 10px">';
if (init ( 'object_id' ) == '') {
	foreach ( $allObject as $object ) {
		foreach ( $object->getEqLogic ( true, false, 'chromecast' ) as $chromecast ) {
			echo $chromecast->toHtml ( 'dview' );
		}
	}
} else {
	foreach ( $object->getEqLogic ( true, false, 'chromecast' ) as $chromecast ) {
...
<?php include_file('desktop', 'panel', 'js', 'chromecast');?>